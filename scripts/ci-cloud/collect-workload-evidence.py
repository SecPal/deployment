...
This file is the reviewed target-side entrypoint. The orchestrator streams it
...
1. The workload contract: the reviewed roles, unit names, networks, volumes,
   ...
   WORKLOAD_CONTRACT_EXPORTS are the authoritative definition of every concept
...
a target can never grade itself.
"""
...
# The trusted workload contract surface. Every name here is defined once in
# this file and bound verbatim by the controller-side admission layer, which
# refuses to load when the two lists diverge. Adding a shared concept means
# declaring it here rather than restating it in the admission layer.
...
# live in its own module because this file is streamed to the target, so
# tests/ci-cloud-workload-layers.py enforces the boundary instead: it proves
# that none of these functions can reach the system, directly or through a
# callee. A normalization helper that needs to observe something belongs in
# the collection layer below, not here.
REPRESENTATION_NORMALIZATION_EXPORTS = (
    "canonical_systemd_unit_name",
    "configured_id_maps",
    "configured_userns_options",
    "container_pid_matches_state",
    "direct_podman_exec_start",
    "exact_systemd_service_properties",
    ...
    "normalized_command",
    "normalized_mounts",
    "normalized_network_address",
    "normalized_network_endpoints",
    "normalized_service_environment",
    "normalized_tmpfs",
    "parse_id_map",
    "parsed_manager_environment",
    "parsed_tmpfs_size",
    "podman_health_exec_is_trusted",
    "podman_health_timer_interval_usec",
    "podman_lifecycle_exec_is_trusted",
    "quadlet_content_has_no_auxiliary_execution_directives",
    "service_config_environment_is_trusted",
    "service_environment_assignments",
    "service_environment_controls_are_trusted",
    "service_execution_controls_are_trusted",
    "service_runtime_controls_are_trusted",
    "service_state_matches_role",
)
...
def canonical_systemd_unit_name(value: object) -> bool:
    """Return whether a value is a canonical systemd unit or job name.

    This is the one place the collection layer decides what a systemd name may
    look like. The closed evidence schema states the same bound and alphabet
    once, as ``$defs/canonicalSystemdUnitName``, and
    ``tests/ci-cloud-workload-layers.py`` proves the two agree name for name.
    A collector that admits an escape the schema rejects is how evidence
    stopped validating before, so the rule must never be restated inline.
    """
    return (
        isinstance(value, str)
        and 1 <= len(value) <= CANONICAL_SYSTEMD_NAME_LIMIT
        and re.fullmatch(
...
def user_work_facts() -> tuple[dict[str, object], bool]:
    ...
    for line in units_output.splitlines():
        fields = line.split()
        if not fields or not canonical_systemd_unit_name(fields[0]):
            complete = False
            continue
    ...
    for line in jobs_output.splitlines():
        ...
        if (
            ...
            or not canonical_systemd_unit_name(fields[1])
        ):
            complete = False
            continue
