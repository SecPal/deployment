...
variable "cpu_profile" {
  description = "Closed DigitalOcean premium CPU profile."
  type        = string

  validation {
    condition     = contains(["intel", "amd"], var.cpu_profile)
    error_message = "cpu_profile must be intel or amd."
  }
}

variable "region" {
  description = "Closed DigitalOcean test region."
  type        = string
